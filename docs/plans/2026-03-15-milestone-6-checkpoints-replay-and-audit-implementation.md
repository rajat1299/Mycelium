...
- `2026-03-16`: Task 2 completed on `codex/m6-task2-checkpoint-persistence`: added `run_checkpoints` and `run_audit_events` persistence, expanded `outcome_runs` with `latest_checkpoint_id` and `resumable`, added checkpoint and audit repositories plus checkpoint-backed run-state restore, and mirrored the new sequence/FK behavior in the fake repository DB.
- `2026-03-16`: Task 3 completed on `codex/m6-task3-checkpoint-runtime`: added a control-plane `checkpoint-service` backed by the local filesystem store, captured checkpoints at `run_started`, `step_completed`, `step_blocked_on_approval`, `approval_resolved`, and terminal boundaries, exposed startup interruption recovery plus manual resume through the execution service, and extended the in-memory/database repository surfaces with checkpoint, audit, run-status scan, and checkpoint-backed restore parity.
- `2026-03-16`: Task 4 completed on `codex/m6-task4-replay-audit-api`: added `GET /api/runs/:runId/checkpoints`, `GET /api/checkpoints/:checkpointId`, `GET /api/runs/:runId/audit`, and `POST /api/runs/:runId/resume`, wired them through the existing checkpoint and execution services, and verified `checkpoint.created` plus `run.resumed` over the live outcome SSE stream.
- `2026-03-16`: Task 5 completed on `codex/m6-task5-checkpoint-ui`: layered checkpoint timeline, selected checkpoint detail, audit trail, and resume controls into the existing outcome console, added same-origin checkpoint or audit proxies, and hardened the console against stale async refreshes and retry dead ends while keeping the M3-M5 timeline, lineage, review, artifact, and log surfaces intact.
- `2026-03-16`: Task 6 completed on `codex/m6-task6-docs-verification`: updated the README, local-dev guide, runbook, roadmap, design doc, and project log for the shipped M6 continuity flow; documented `CHECKPOINT_ROOT` defaulting to `apps/control-plane/.mycelium/checkpoints`; and closed the milestone with a live smoke path that proved interruption recovery, resume from `checkpoint_9e29dc52-f199-499e-a599-f936e5c28067`, no duplicate rerun of the already checkpointed `Analyze outcome` step, rendered `Replay anchors` plus `Operator trail`, and final completion after approval.
