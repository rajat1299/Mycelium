...
Milestone 8 is integrated on `main`. The shipped stack now includes authenticated remote worker sessions, durable schedules, Slack and Telegram ingress, outbound message delivery, message history, and the existing M6 interruption or resume model on top of the Milestone 5 review queue, Milestone 4 routing, and the Milestone 3 local Docker fallback path. Approval resolution remains web-authoritative in M8, and the local companion remains protocol, trust-boundary, and bootstrap groundwork only. Start with the runbook, then the local setup guide, then the M8 design and implementation docs.
