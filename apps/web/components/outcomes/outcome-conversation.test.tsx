...
describe("OutcomeConversation", () => {
  ...
  it("groups each follow-up turn with its own AI work in a dedicated transcript container", () => {
    render(
      <OutcomeConversation
        outcomeId="outcome_123"
        outcomePrompt="Research AI in K-12 education and generate a PDF."
          ...
          nodes: [
            {
              id: "node_followup_b",
              kind: "task",
              title: "Rewrite for cabinet review",
              capability: "document",
              position: 0
            }
          ],
        ...
        initialRun={{
          ...
          status: "running",
          createdAt: "2026-03-22T00:07:00.000Z",
          updatedAt: "2026-03-22T00:07:10.000Z",
          steps: [
            {
              id: "step_followup_b",
              runId: "run_followup_b",
              planNodeId: "node_followup_b",
              title: "Rewrite for cabinet review",
              kind: "task",
              capability: "document",
              status: "running",
              ...
              steps: [
                {
                  ...
                  position: 0,
                  createdAt: "2026-03-22T00:05:00.000Z",
                  updatedAt: "2026-03-22T00:05:50.000Z"
                }
              ]
            },
            {
    ...
    const turns = screen.getAllByTestId("outcome-thread-turn");
    expect(turns).toHaveLength(2);

    const principalsTurn = document.querySelector(
      '[data-testid="outcome-thread-turn"][data-turn-key="turn:prompt"]'
    );
    const cabinetTurn = screen
      .getByText("Now make it shorter for the district cabinet.")
      .closest('[data-testid="outcome-thread-turn"]');

    expect(principalsTurn).not.toBeNull();
    expect(cabinetTurn).not.toBeNull();
    expect(principalsTurn).not.toBe(cabinetTurn);
    expect(principalsTurn).toContainElement(
      screen.getByText("Research AI in K-12 education and generate a PDF.")
    );
    expect(principalsTurn).toContainElement(
      screen.getByText("Here is the shorter principal-ready version.")
    );
    expect(principalsTurn).toContainElement(
      screen.getAllByText("Shorten for principals")[0] ?? null
    );
    expect(principalsTurn).not.toContainElement(
      screen.getByText("I’m trimming it down even further for cabinet review.")
    );
    expect(cabinetTurn).toContainElement(
      screen.getByText("I’m trimming it down even further for cabinet review.")
    );
    expect(cabinetTurn).toContainElement(
      screen.getAllByText("Rewrite for cabinet review")[0] ?? null
    );
  });
