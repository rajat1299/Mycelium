...
export async function PUT(request: Request) {
  try {
    ...
    if (!parsed.success) {
      return jsonResponse({ error: "Invalid Telegram connection payload." }, 400);
    }

    const { workspaceId, ...body } = parsed.data;
    const upstream = await fetch(
      `${getControlPlaneBaseUrl()}/api/workspaces/${encodeURIComponent(workspaceId)}/telegram/connection`,
      {
        method: "PUT",
        headers: {
    ...
    return jsonResponse({ error: "Unable to save Telegram connection." }, 502);
  }
}
